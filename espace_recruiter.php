...
// Suppression d'un job et ses candidatures
if (isset($_GET['delete_id'])) {
    ...
    try {
        ...
        // Récupérer les fichiers CV et lettres de motivation associés
        $query_get_files = "SELECT cv_file, cover_letter_file FROM applications WHERE job_id = :job_id";
        $stmt_get_files = $conn->prepare($query_get_files);
        $stmt_get_files->execute([':job_id' => $delete_id]);

        // Supprimer les fichiers du dossier "uploads"
        while ($file = $stmt_get_files->fetch(PDO::FETCH_ASSOC)) {
            if (!empty($file['cv_file']) && file_exists("uploads/" . $file['cv_file'])) {
                unlink("uploads/" . $file['cv_file']); // Supprime le fichier CV
            }
            if (!empty($file['cover_letter_file']) && file_exists("uploads/" . $file['cover_letter_file'])) {
                unlink("uploads/" . $file['cover_letter_file']); // Supprime la lettre de motivation
            }
        }

        // Supprimer les candidatures associées au job
        $query_delete_applications = "DELETE FROM applications WHERE job_id = :job_id";
        $stmt_delete_applications = $conn->prepare($query_delete_applications);
        $stmt_delete_applications->execute([':job_id' => $delete_id]);

        // Supprimer le job lui-même
        $query_delete_job = "DELETE FROM jobs WHERE id = :id";
        $stmt_delete_job = $conn->prepare($query_delete_job);
        $stmt_delete_job->execute([':id' => $delete_id]);

        // Valider la transaction
        $conn->commit();

        $message = '<div class="alert alert-success">Le job, les candidatures associées et leurs fichiers ont été supprimés avec succès.</div>';
