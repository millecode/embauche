<?php
$titre="Gestions des utilisateurs";
...
<main>
                            ...
                            <tbody>
                                <?php foreach ($users as $user): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <input type="checkbox" id="status" name="status" value="1" data-user-id="<?= $user['id']; ?>" <?= ($user['status'] == 1) ? 'checked' : ''; ?>>
                                        </td>
    ...
    <script>
        ...
        document.getElementById('status').addEventListener('change', function() {
            var status = this.checked ? 1 : 0; // Détermine le statut (coché ou décoché)
            var userId = this.getAttribute('data-user-id'); // Récupère l'ID utilisateur à partir de l'attribut data

            // Envoi de la requête AJAX
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "update_status_user.php", true); // Fichier PHP qui met à jour la base
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

            // Traitement de la réponse
            xhr.onload = function() {
                if (xhr.status === 200) {
                    alert("Le statut a été mis à jour.");
                } else {
                    alert("Erreur lors de la mise à jour.");
                }
            };

            // Envoi des données (status et id de l'utilisateur)
